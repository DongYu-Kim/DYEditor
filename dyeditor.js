...
export default function DYEditor ({data, readOnly, imageUploader}) {
    ...
    useEffect(() => {
        if(DYEditorEl.state === undefined) {
            ...
            .then(editor => {
                ...
                else if(imageUploader !== null) { // if imageUpload is null, use Base64 uploader.
                    if(typeof imageUploader === 'function') setUploadImages(_editor, imageUploader);
                    else console.error(`imageUploader should be null or a function that takes a file as input and a imageUrl as output.`);
                }
    ...
    if(_editor) _editor.setData(data);
